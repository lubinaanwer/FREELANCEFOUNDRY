<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
    pageEncoding="ISO-8859-1" import="java.util.*,ff.bl.*,ff.dto.*,ff.dl.*,ff.aig.*,java.util.regex.*"%>
<!DOCTYPE html>
<html lang="en">
  <head>
    <title>Freelance &mdash;Foundry &mdash;feedback</title>
    <meta charset="utf-8">
  ...
  <div class="site-wrap">
	...
	String seeker_id;
 	ArrayList<HiresDTO> hirers=null;%>
		...
		<% 
			seeker_id=(String)session.getAttribute("id");
			hirers=HiresBL.generateFeedback(seeker_id);
			if(hirers!=null &&(hirers.size()>0))
		{
		%> 
			<%! 
				String feedback_date=DateGenerator.generateDate();
			%>
              <form name="myform" action="insert_feedback.jsp" class="p-5 bg-white">

              <div class="row form-group">
                <div class="col-md-12 mb-3 mb-md-0">
                  <label class="font-weight-bold" for="fullname">SEEKER ID</label>
                  <input name="SeekerId" id="SeekerId" readonly="readonly" value="<%=seeker_id%>" class="form-control">
                </div>
              </div>
              <div class="row form-group">
                <div class="col-md-12 mb-3 mb-md-0">
                  <label class="font-weight-bold" for="fullname">WORKER ID</label>
                  	<select class="form-control" name="WorkerId"id="WorkerId">
                  	<%for(HiresDTO hirer:hirers)
					        	  {%>
					        	  <option value="<%=hirer.getWorkerId()%>"><%=hirer.getWorkerId()%></option>
					        	  <%} %>
                  	</select>
                </div>
              </div>
              <div class="row form-group">
                <div class="col-md-12">
                  <label class="font-weight-bold" for="email">DATE</label>
                  <input class="form-control" name="FeedbackDate" readonly="readonly" id="FeedbackDate" value="<%=feedback_date%>">
                </div>
              </div>
              <div class="row form-group">
                <div class="col-md-12">
                  <label class="font-weight-bold" for="email">Feedback</label>
